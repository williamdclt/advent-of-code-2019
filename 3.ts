...
function doTheJob(firstInput: string[], secondInput: string[]) {
  const firstWirePath = {};
  const crossings = [];

  const getKey = pos => `${pos[0]},${pos[1]}`;
  function registerMove(pos, steps) {
    const key = getKey(pos);
    firstWirePath[key] = steps;
    return pos;
  }

  function getNewPosition(lastPosition, direction) {
    if (direction === "U") return [lastPosition[0], lastPosition[1] + 1];
    if (direction === "D") return [lastPosition[0], lastPosition[1] - 1];
    if (direction === "R") return [lastPosition[0] + 1, lastPosition[1]];
    if (direction === "L") return [lastPosition[0] - 1, lastPosition[1]];
  }

  let lastPosition = [0, 0];
  let moves = 0;
  for (let move of firstInput) {
    const direction = move[0];
    const distance = Number.parseInt(move.substr(1));

    for (let i = 1; i <= distance; i++) {
      moves++;
      const newPosition = getNewPosition(lastPosition, direction);
      registerMove(newPosition, moves);
  ...
  lastPosition = [0, 0];
  moves = 0;
  for (let move of secondInput) {
    const direction = move[0];
    const distance = Number.parseInt(move.substr(1));

    for (let i = 1; i <= distance; i++) {
      moves++;
      const position = getNewPosition(lastPosition, direction);
      const firstWireSteps = firstWirePath[getKey(position)];
      if (firstWireSteps) {
        crossings.push({ position, steps: firstWireSteps + moves });
      }
  ...
  const manhattan = pos => Math.abs(pos[0]) + Math.abs(pos[1]);
  const minCrossing = crossings.reduce(
    (minCross, crossing) => Math.min(minCross, manhattan(crossing.position)),
    Infinity
  );
  const minSteps = crossings.reduce(
    (minMoves, crossing) => Math.min(minMoves, crossing.steps),
    Infinity
  );
  return { minCrossing, minSteps };
}

assertEqual(doTheJob("R8,U5,L5,D3".split(","), "U7,R6,D4,L4".split(",")), {
  minCrossing: 6,
  minSteps: 30
});
